...
window.onerror = function (message, source, lineno, colno, error) {
    ...
    if (el) {
        ...
        el.innerHTML = `<strong>Hiba történt:</strong><br>${message}`;
    }
};
...
const distortions = {
    turbulentDistortion: {
        ...
        getJS: (progress, time) => {
            ...
            const getY = p => - (Math.sin(Math.PI * p * uFreq.z + time) * 0.5 + 0.5) * uAmp.z - Math.pow((Math.sin(Math.PI * p * uFreq.w + time / (uFreq.z / uFreq.w)) * 0.5 + 0.5), 5) * uAmp.w;
...
try {
    ...
    if (!container) throw new Error("A konténer (div id='lights') nem található! Másold be a teljes HTML kódot.");

    // Konfiguráció javítása
    ...
    myApp.loadAssets().then(myApp.init).catch(err => {
        ...
        if (errEl) {
            ...
            errEl.innerHTML = "Hiba az assetek betöltésekor: " + err.message;
        }
    });
} catch (err) {
    console.error(err);
    ...
    if (errEl) {
        errEl.style.display = 'block';
        errEl.innerHTML = "Kritikus hiba: " + err.message;
    }
}